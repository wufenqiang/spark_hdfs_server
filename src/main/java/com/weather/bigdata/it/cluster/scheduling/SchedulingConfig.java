package com.weather.bigdata.it.cluster.scheduling;

import com.weather.bigdata.it.cluster.db.localDB;
import com.weather.bigdata.it.cluster.util.CasicProperties;
import com.weather.bigdata.it.spark.sparksubmit.monitorHDFS;
...
public class SchedulingConfig {
    ...
    private static boolean open=Boolean.valueOf(CasicProperties.CONFPROPERTIES.getProperty("sparksubmit_open"));
    ...
    public void run_monitorHDFS() {
        if(this.open){
            ...
            String msg="定时任务开始["+timeStamp+"]";
            logger.info(msg);

            String splitFile= localDB.getSplitFile();

            monitorHDFS.HDFSLister(splitFile);

            msg="定时任务结束["+timeStamp+"]";
            logger.info(msg);
        }else{
            logger.info("monitorHDFS配置关闭,open="+this.open);
        }
    }
}
//
